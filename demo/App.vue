...
import { plugin, executeMusic } from "../src/vue";
import type { ToolResult, ToolSample, ToolPlugin } from "gui-chat-protocol/vue";
import type { MusicArgs, MusicToolData } from "../src/core/types";
...
const result = ref<ToolResult<MusicToolData>>({
...
const executeSample = async (sample: ToolSample) => {
  const args = sample.args as unknown as MusicArgs;
  const execResult = await executeMusic({} as never, args);
  result.value = {
    ...result.value,
    ...execResult,
    uuid: `demo-${Date.now()}`,
